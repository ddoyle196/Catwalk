...
const StarRating = ({ ratings }) => {
  ...
  while (blackStars > 0.00) {
    if (blackStars >= 1.00) {
      stars.push(<InlineIcon key={keyCount += 1} icon={Q4Star} />);
      blackStars -= 1.00;
    } else if (blackStars >= 0.75) {
      stars.push(<InlineIcon key={keyCount += 1} icon={Q3Star} />);
      blackStars -= 0.75;
    } else if (blackStars >= 0.50) {
      stars.push(<InlineIcon key={keyCount += 1} icon={Q2Star} />);
      blackStars -= 0.50;
    } else if (blackStars >= 0.25) {
      stars.push(<InlineIcon key={keyCount += 1} icon={Q1Star} />);
      blackStars -= 0.25;
    }
  }
  while (whiteStars >= 1) {
    stars.push(<InlineIcon key={keyCount += 1} icon={Q0Star} />);
    whiteStars -= 1.00;
  }
